...
function confirmArchive(service, id, patientFirstName, patientLastName, userFirstName , userLastName , userPosition) {
    Swal.fire({
        icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            window.location.href = 
                '/barangay-datu-esmael-rms/dist/includes/delete_query.php?' + 'archive&' +
                service + 
                '&id=' + id + 
...
function confirmRestore(service, id, patientFirstName, patientLastName, userFirstName , userLastName , userPosition) {
    Swal.fire({
        icon: 'question',
        ...
        if (result.isConfirmed) {
            // form.submit();
            window.location.href = 
                '/barangay-datu-esmael-rms/dist/includes/delete_query.php?' + 'restore&' +
                service + 
                '&id=' + id + 
                '&patientFirstName=' + patientFirstName + 
                '&patientLastName=' + patientLastName + 
...
function addUser() {
        Swal.fire({
                ...
                showCancelButton : true,
                preConfirm: () => {
                    const defaultUsername = Swal.getPopup().querySelector('#bhw-contact').value
                    const defaultPassword = Swal.getPopup().querySelector('#bhw-password').value
                    ...
                    return { defaultUsername: defaultUsername, defaultPassword: defaultPassword, role: role }
                }
        }).then((result) => {
                    window.location.href = '/barangay-datu-esmael-rms/dist/includes/add_query.php?save_bhw&bhw-contact=' + result.value.defaultUsername + '&bhw-pass=' + result.value.defaultPassword + '&bhw-role=' + result.value.role
...
function confirmDelete(service, id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = 
                '/barangay-datu-esmael-rms/dist/includes/delete_query.php?' + 'delete&' +
                service + 
                '&id=' + id
        } else {
            return Swal.close()
