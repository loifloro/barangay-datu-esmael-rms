    ...
    ?>
            <ul class="patient-profile__list" role="list">
                ...
                <ul class="patient-profile__item " role="list">
                    ...
                    $ec_bdate = new DateTime($patient['child_birthdate']);
                    $new_ec_bdate = $ec_bdate->format("m-d-Y");
                    ?>
...
<section class="history">
    <section class="medical-history">
        ...
        <div class="medical-history__title">
            ...
            <button class="btn-green btn-change btn-restore" onclick="addNewRecord('<?= $patient['child_fname'] ?>' , '<?= $patient['child_lname'] ?>' , '<?= $patient['phone_num']; ?>' , '<?= $patient['child_birthdate'];; ?>' ,  '<?= $patient['sex']; ?>' , '<?= $patient['street_address']; ?>' , '<?= $patient['city']; ?>' , '<?= $patient['barangay'] ?>' , '<?= $patient['early_childhood_email']; ?>')">
                <p>Add Service</p>
            </button>
        </div>
        ...
        <div class="medical-history__card">
            ...
            ?>
                    <ul class="medical-history__card__row" role="list">
                        ...
                        <?php
                        // CONVERT DATE TO MM-DD-YY
                        $ec_ldate = new DateTime($recent3['deworming_date']);
                        $new_ec_ldate = $ec_ldate->format("m-d-Y");
                        ?>
                    ...
                    if (isset($_GET['fname']) && isset($_GET['lname'])) {
                        ...
                        if (mysqli_num_rows($query_run2) > 0) {
                            foreach ($query_run2 as $recent) {
                                // CONVERT DATE TO MM-DD-YY
                                $ec_rdate = new DateTime($recent['date_edit']);
                                $new_ec_rdate = $ec_rdate->format("m-d-Y");
                    ?>
                                ...
                                <span class="edit-history__date"><?= $new_ec_rdate; ?></span>
                                <hr>
                    <?php
                            }
                        }
