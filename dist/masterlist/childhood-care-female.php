...
<head>
    ...
    <title>Masterlist Child Care Female</title>
</head>
...
<body class="grid">
    ...
    <main class="masterlist">
        ...
        <div class="masterlist__table">
            ...
            <!-- START QUERY -->
            <?php
            $query = "SELECT * FROM target_childcare_female"; // WHERE archive_label=''
            $query_run = mysqli_query($conn, $query);
            if (mysqli_num_rows($query_run) > 0) {
                if (isset($_POST['sort_name'])) {
                    $sort_id = $_POST['sort_name'];
                    if ($sort_id == 1) {
                        $query = "SELECT * FROM target_childcare_female ORDER BY child_firstname";
                        $query_run = mysqli_query($conn, $query);
                    }
                }
                if (isset($_POST['sort_date'])) {
                    ...
                    if ($sort_id == 2) {
                        $query = "SELECT * FROM target_childcare_female ORDER BY date_registered";
                        $query_run = mysqli_query($conn, $query);
                    }
                }
                if (isset($_POST['sort_address'])) {
                    ...
                    if ($sort_id == 3) {
                        $query = "SELECT * FROM target_childcare_female ORDER BY complete_address";
                        $query_run = mysqli_query($conn, $query);
                    }
                }
                if (isset($_POST['sort_bdate'])) {
                    ...
                    if ($sort_id == 3) {
                        $query = "SELECT * FROM target_childcare_female ORDER BY birthday";
                        $query_run = mysqli_query($conn, $query);
                    }
                }
        ...
        <button type="submit" class="btn-green btn-add services__btn" onclick="window.location.href = '../add/add-child_care-female.php'">
            <p>Add</p>
        </button>
    </main>
