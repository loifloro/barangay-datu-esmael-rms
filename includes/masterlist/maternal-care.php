...
<main class="masterlist">
    ...
    <div class="masterlist__table">
        ...
        $query = "SELECT * FROM target_maternal LIMIT $start_from, $num_per_page"; // WHERE archive_label=''
        $query_run = mysqli_query($conn, $query);
        if (mysqli_num_rows($query_run) > 0) {
            if (isset($_POST['sort_name'])) {
                $sort_id = $_POST['sort_name'];
                if ($sort_id == 1) {
                    $query = "SELECT * FROM target_maternal ORDER BY firstname LIMIT $start_from, $num_per_page";
                    $query_run = mysqli_query($conn, $query);
                }
            }
            if (isset($_POST['sort_date'])) {
                ...
                if ($sort_id == 2) {
                    $query = "SELECT * FROM target_maternal ORDER BY date_registered LIMIT $start_from, $num_per_page";
                    $query_run = mysqli_query($conn, $query);
                }
            }
            if (isset($_POST['sort_address'])) {
                ...
                if ($sort_id == 3) {
                    $query = "SELECT * FROM target_maternal ORDER BY complete_address LIMIT $start_from, $num_per_page";
                    $query_run = mysqli_query($conn, $query);
                }
            }
            if (isset($_POST['sort_bdate'])) {
                ...
                if ($sort_id == 3) {
                    $query = "SELECT * FROM target_maternal ORDER BY birthday LIMIT $start_from, $num_per_page";
                    $query_run = mysqli_query($conn, $query);
                }
            }
        ...
        ?>
                <ul class="masterlist__table__row masterlist__info" role="list">
                    ...
                    <li class="masterlist__option">
                        <button type="button" name="delete_maternal" value="<?= $patient['target_maternal_id']; ?>" onclick="confirmDelete('delete_maternal' , '<?= $patient['target_maternal_id']; ?>')">
                        ...
                        <button type="button" onclick="window.location.href = `edit-record.php?service=maternal-care&id=<?= $patient['target_maternal_id']; ?>`">
            ...
            //PAGINATION
            $pr_query = "SELECT * FROM target_maternal";
            ...
            <div class="pagination">
                ...
                if ($page > 1) {
                ?>
                    <a href='masterlist.php?maternal-care&page=<?php echo ($page - 1) ?>' class="pagination_previous">Previous</a>
                <?php
                }
                for ($i = 1; $i <= $total_page; $i++) {
                ?>
                    <a href='masterlist.php?maternal-care&page=<?php echo $i; ?>' class="pagination_number"><?php echo $i; ?></a>
                <?php
                }
                if ($page < $total_page) {
                ?>
                    <a href='masterlist.php?maternal-care&page=<?php echo ($page + 1) ?>' class="pagination_next">Next</a>
                <?php
                }
                //END OF PAGINATION
            } else {
            ...
            <button type="submit" class="btn-green btn-add services__btn" onclick="window.location.href = 'add-record.php?service=maternal-care'">
                <p>Add</p>
            </button>
    </div>
</main>